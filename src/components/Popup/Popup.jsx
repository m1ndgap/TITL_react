...
const Popup = () => {
    ...
    return (
        <div className={`popupEl popup-el 
            ${state.active ? "" : "popup-el--hidden"}
            ${state.theme === "dark" ? "popup-el--dark-theme" : ""}
            ${state.size === "large" ? "popup-el--large" : ""}
            `}>
            <span className={
                    `popup-header 
                    ${state.active ? "" : "popup-header--hidden"}
                    ${state.theme === "dark" ? "" : "popup-header--dark-theme"}`
                }>
                {state.title}
            </span>
            <span className={
                    `popup-text
                    ${state.active ? "" : "popup-text--hidden"}
                    ${state.theme === "dark" ? "" : "popup-text--dark-theme"}`
                }>
                {state.text}
            </span>
            <div className={
                `popup-link
                    ${state.active ? "" : "popup-link--hidden"}`
            }>
